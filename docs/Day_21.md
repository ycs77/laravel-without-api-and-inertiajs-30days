...
為了不要讓任何人都能隨便更新或刪除，授權的規則是確認當前用戶是否為文章作者：
...
會影響到的有頁面標題、送出按鈕文字、送出路徑等，還有更新的路由是 PUT 方法，FormData 要 append `_method=put` 的值。最後在葉面標題下還增加一排導覽列，方便回到文章或列表：
...
文章基本的 CRUD 操作均已備齊，即將前往下一個功能 - 點讚 (但在此系列我會叫 `喜歡`)，當前網路環境最熱門的互動功能。下篇將會實作此喜歡文章功能。
